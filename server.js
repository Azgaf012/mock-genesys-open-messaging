...
app.post("/api/v2/conversations/messages/:integrationId/inbound/open/message", (req, res) => {
  ...
  log("INFO", "Inbound Open Messaging request received", {
    integrationId,
    headers: {
      authorization: req.headers["authorization"] ? req.headers["authorization"].replace(/Bearer\s+\S+/, "Bearer [REDACTED]") : undefined,
  ...
  const response = {
    ...
    channel: {
      ...
      from: {
        ...
        // image omitido intencionalmente
      },
    },
    type: "Text",
    text: body.text ?? "",
